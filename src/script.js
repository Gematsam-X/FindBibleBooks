...
function checkBibleBook() {
  ...
  if (matches.length === 1 || sanitizedInput === "salmo") {
    ...
    if (!chapterInput || isNaN(chapterInput)) {
      ...
      injectOutput(`https://www.jw.org/finder?wtlocale=I&prefer=lang&bible=${bookNumber}${chapterNumber}${verseNumber}-${bookNumber}${chapterNumber}${verseNumber}&pub=nwtsty`);
    }
